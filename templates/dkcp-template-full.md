## Введение

Этот шаблон нужно использовать всегда, когда есть необходимость вывести в документ ценностное предложение по технологии ДКЦП


## Структура документа
- Ценностное предложение 
	1. Название ценностного предложения как ты его понял
	2. Клиентский сегмент
		1. Должно быть указано название и критерии
	3. мотивационный конфликт или конфликты
		1. Должны быть расписаны все элементы "грозовой тучи"
		2. Должна быть проведена проверка на конфликтность, сформулированы противоречия первого и второго порядка (если есть)
		3. Должны быть перечисленны все установки, поддающиеся разрушению
	4. Структура атрибутов и артефактов
		1. Общий список артефактов и их атрибутов
		2. Таблица синтеза со следующими столбцами: Артефакт-носитель | Атрибут артефакта | Ситуация проявления опыта | Репрезентативная система(ы) | Опыт клиента | Связь с конфликтом
	5. Ценовые атрибуты и условия обмена
		1. Должен быть указан материальный и нематериальный вклад
	6. Ценности
		1. Перечисление
	7. Атрибуты компании
		1. Перечисление
	8. Аргументы
		1. Аргумент о (далее о взаимосвязи каких блоков аругмент)
